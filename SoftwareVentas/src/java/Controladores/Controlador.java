        ...
        if (menu.equals("Empleados")) {
            ...
            switch (accion) {
                ...
                case "Actualizar":
                    ...
                    usuario1.setId(idUsuario); // capta el idUsuario del metodo cargar
                    usuarioDAO.Actualizar(usuario1);
                    request.getRequestDispatcher("Controlador?menu=Empleados&accion=Listar").forward(request, response);

                    break;
                case "Cargar":  //captando el id
        ...
        if (menu.equals("Ventas")) {
            switch (accion) {
                case "BuscarCliente":
                    ...
                    request.setAttribute("cliente", usuario); //enviar el parametro cliente  a la vista
                    break;
                ...
                case "BuscarProducto":
                    ...
                    request.setAttribute("cliente", usuario);// se  envia cliente  para que no se elimine
                    for (int i = 0; i < listaVentas.size(); i++) {
                        totalapagar += listaVentas.get(i).getSubtotal();
                    }
                    formatoNumero1 = NumberFormat.getNumberInstance();
                    total1 = formatoNumero1.format(totalapagar); // se envia para que cada vez  busque producto no borre el valor
                ...
                case "AgregarProducto":
                    ...
                    request.setAttribute("listaventas", listaVentas); // enviar atributo a  la vista  al cforeach
                    for (int i = 0; i < listaVentas.size(); i++) {
                        totalapagar += listaVentas.get(i).getSubtotal();// suma el total a pagar de los subtatales de los registros
                    }
                    formatoNumero1 = NumberFormat.getNumberInstance();
                ...
                default:
                    ...
                    request.setAttribute("numerofactura", numfac);// parametro que se  envia numerofactura  al txtnumerofactura
